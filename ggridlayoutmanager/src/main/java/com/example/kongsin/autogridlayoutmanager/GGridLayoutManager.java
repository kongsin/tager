 ...
 * </ul>
 */
public class GGridLayoutManager extends RecyclerView.LayoutManager {

    private static final String TAG = GGridLayoutManager.class.getSimpleName();
    ...
    private void fillGrid(int direction, int emptyLeft, int emptyTop,
        ...
        for (int i = 0; i < getVisibleChildCount(); i++) {
            ...
            if (view == null) {
                ...
                view.getLayoutParams().width =  (mDecoratedChildWidth / getTotalColumnCount());
                view.getLayoutParams().height = LayoutParams.WRAP_CONTENT;
                measureChildWithMargins(view, 0, 0);
                Log.i(TAG, "fillGrid: -------------START----------");
                Log.i(TAG, "fillGrid: x " + leftOffset);
                Log.i(TAG, "fillGrid: y " + topOffset);
                Log.i(TAG, "fillGrid: w " + (mDecoratedChildWidth / getTotalColumnCount()));
                Log.i(TAG, "fillGrid: h " + mDecoratedChildHeight);
                Log.i(TAG, "fillGrid: -------------END----------");
                layoutDecorated(view, leftOffset, topOffset, (mDecoratedChildWidth / getTotalColumnCount()), mDecoratedChildHeight);
            ...
            int nx = ((i+1) % getTotalColumnCount());
            if (nx == 0) {
                leftOffset = emptyLeft;
                topOffset += mDecoratedChildHeight;
            } else {
                leftOffset += (mDecoratedChildWidth / getTotalColumnCount());
                topOffset = mDecoratedChildHeight;
            }
        }
        for (int i=0; i < viewCache.size(); i++) {
